...
it('delivers mapped events to the Google transport under its registered name', function (): void {
    ...
    $transport->shouldReceive('send')->once()->with(Mockery::on(fn (array $payload): bool => $payload['name'] === 'purchase'))->andReturn(['ok' => true]);
